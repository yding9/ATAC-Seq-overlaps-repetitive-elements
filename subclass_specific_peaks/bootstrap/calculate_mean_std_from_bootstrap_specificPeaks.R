# one tip from JohnM: row_name_temp %in% row_names.
  ...
  for (i in seq_along(inFileNames)){
    if (index <= 7) {
      if (grepl(file_loop[index], list_strings[[i]][5])){
        ...
        row_name_temp <- paste0(substring(list_strings[[i]][1],3),"_", substring(list_strings[[i]][5],3))
      }
    } else {
      if (grepl("shuffled", list_strings[[i]][5])){
        row_name_temp <- paste0(substring(list_strings[[i]][1],3),"_All")
      } else {
        row_name_temp <- paste0(substring(list_strings[[i]][1],3),"_", substring(list_strings[[i]][5],3))
      }
...
write.table(result, file = "specific_STD_MEAN_includeShuffle_matrix.xls", sep = "\t")
